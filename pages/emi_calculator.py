"""
Loan Eligibilty and EMI Prediction AI - EMI Calculator Page
...
st.caption(
    "Fill in the applicant's details below to estimate the largest monthly EMI they can safely sustain."
)
...
if calculate_clicked:
    ...
    X_input = (
        models["scaler"].transform(feature_row) if needs_scaling(reg) else feature_row
    )
    ...
    if requested_installment <= predicted_emi:
        ...
        st.write(
            f":material/info: At the requested {raw['requested_tenure']}-month tenure, a loan amount up to "
    ...
    c1.metric(
        "Monthly Salary",
        f"\u20b9{raw['monthly_salary']:,.0f}",
        icon=":material/payments:",
    )

    c2.metric(
        "Disposable Income",
        f"\u20b9{feature_row['disposable_income'].iloc[0]:,.0f}",
        icon=":material/account_balance_wallet:",
    )

    c3.metric(
        "Affordability Ratio",
        f"{feature_row['affordability_ratio'].iloc[0] * 100:.1f}%",
