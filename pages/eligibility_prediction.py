"""
Loan Eligibilty and EMI Prediction AI - Eligibility Prediction Page
...
st.caption(
    "Fill in the applicant's details below, then click Predict to get an eligibility decision."
)
...
if predict_clicked:
    ...
    X_input = (
        models["scaler"].transform(feature_row) if needs_scaling(clf) else feature_row
    )
    ...
    result_style = {
        "Eligible": (
            ...
            "green",
            "This applicant looks eligible for the requested EMI.",
        ),
        "High_Risk": (
            ":material/warning:",
            "orange",
            "This applicant is borderline - recommended for manual review.",
        ),
        "Not_Eligible": (
            ":material/cancel:",
            "red",
            "This applicant does not currently meet the eligibility bar.",
        ),
    }
    ...
    c1.metric(
        "Eligible",
        f"{proba_by_class['Eligible'] * 100:.1f}%",
        icon=":material/check_circle:",
    )

    c2.metric(
        "High Risk",
        f"{proba_by_class['High_Risk'] * 100:.1f}%",
        icon=":material/warning:",
    )

    c3.metric(
        "Not Eligible",
        f"{proba_by_class['Not_Eligible'] * 100:.1f}%",
    ...
    with st.expander("Why did the model decide this?", icon=":material/lightbulb:"):
        ...
        st.dataframe(
            feature_row.T.rename(columns={0: "Value"}), use_container_width=True
        )
