"""
Loan Eligibilty and EMI Prediction AI - Applicant Input Form
...
def render_applicant_form():
    ...
    age = c1.number_input(
        "Age", min_value=21, max_value=65, value=int(ss.get("applicant_age", 35))
    )
    ...
    emi_scenario = c1.selectbox(
        "Loan Type",
        EMI_SCENARIOS,
        index=EMI_SCENARIOS.index(
            ss.get("applicant_emi_scenario", "Personal Loan EMI")
        ),
    )
    ...
    raw = {
        ...
        "current_emi_amount": float(
            current_emi_amount if existing_loans == "Yes" else 0
        ),
