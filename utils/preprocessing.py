"""
Loan Eligibilty and EMI Prediction AI - Preprocessing
...
def build_feature_row(raw, feature_names):
    ...
    total_monthly_expenses = (
        ...
        + raw["college_fees"]
        + raw["travel_expenses"]
        + raw["groceries_utilities"]
        + raw["other_monthly_expenses"]
        + raw["monthly_rent"]
    )
    ...
    row["debt_to_income_ratio"] = round(
        raw["current_emi_amount"] / raw["monthly_salary"], 4
    )

    row["expense_to_income_ratio"] = round(
        total_monthly_expenses / raw["monthly_salary"], 4
    )

    disposable_income = (
        raw["monthly_salary"] - total_monthly_expenses - raw["current_emi_amount"]
    )
    ...
    row["emergency_fund_months"] = (
        round(raw["emergency_fund"] / total_monthly_expenses, 2)
        if total_monthly_expenses
        else 0
    )

    row["liquidity_ratio"] = (
        round(raw["bank_balance"] / total_monthly_expenses, 2)
        if total_monthly_expenses
        else 0
    )
    ...
    band = pd.cut([raw["credit_score"]], bins=CREDIT_BINS, labels=CREDIT_LABELS)[0]
